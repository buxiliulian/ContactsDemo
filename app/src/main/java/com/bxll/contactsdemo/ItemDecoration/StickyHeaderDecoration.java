...
import com.bxll.contactsdemo.R;
...
public class StickyHeaderDecoration extends RecyclerView.ItemDecoration {
    ...
    private StickyHeaderDecoration(Builder builder) {
        ...
        mHeaderHeight = (int) Math.ceil(fontMetrics.bottom - fontMetrics.top);

        final TypedArray a = context.obtainStyledAttributes(ATTRS);
        mDivider = a.getDrawable(0);
        if (mDivider == null) {
            Log.w(TAG, "@android:attr/listDivider was not set in the theme used for StickyHeaderDecoration.");
        }
        a.recycle();
    }
    ...
    public void getItemOffsets(@NonNull Rect outRect, @NonNull View view, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        if (mOrientation == VERTICAL) {
            ...
            if (mCallback.hasHeader(current)) {
                outRect.set(0, mHeaderHeight, 0, 0);
            }

            if (!mCallback.hasHeader(current + 1)) {
    ...
    public void onDraw(@NonNull Canvas c, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        ...
        if (mOrientation == VERTICAL) {
            ...
            for (int i = 0; i < parent.getChildCount(); i++) {
                ...
                // 绘制分割线
                if (!mCallback.hasHeader(position + 1)) {
                    mDivider.setBounds(layoutManager.getDecoratedLeft(child),
                            layoutManager.getDecoratedBottom(child),
                            layoutManager.getDecoratedRight(child),
                            layoutManager.getDecoratedBottom(child) + mDivider.getIntrinsicHeight());
                    mDivider.draw(c);
                }
            }
